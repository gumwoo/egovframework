# 6. 보안 (Security) - 실무 필수 사항 (18강 ~ 22강)
...
```sql
-- 정상 쿼리
SELECT * FROM BOARD WHERE BORDER_ID = 10

-- 공격자가 "10; DROP TABLE BOARD--" 입력
SELECT * FROM BOARD WHERE BORDER_ID = 10; DROP TABLE BOARD--
...
public class BoardServiceImpl {
    ...
    private static final List<String> ALLOWED_SORT_COLUMNS =
        Arrays.asList("BORDER_ID", "TITLE", "WRITE_DAY", "SEE_COUNT");
...
// 실행되는 쿼리:
// SELECT * FROM BOARD WHERE BORDER_ID IN (?, ?, ?, ?, ?)
```
...
**이 문서의 내용을 적용하면 실무 적합도 75점 → 95점으로 향상됩니다!**
