...
CREATE TABLE EGOV_BBS (
    BORDER_ID      SERIAL PRIMARY KEY,      -- 게시글 고유 번호 (자동증가)
    BORDER_TYPE    VARCHAR(1) DEFAULT 'A',  -- 게시판 타입 (A:일반, B:공지 등)
...
**Mapper XML**
```xml
<insert id="insertBorder" parameterType="borderVO">
    <!-- 1. 시퀀스에서 다음 번호를 미리 가져옴 (PostgreSQL 방식) -->
    <selectKey keyProperty="borderId" resultType="int" order="BEFORE">
        SELECT nextval('egov_bbs_border_id_seq')
    </selectKey>

    INSERT INTO EGOV_BBS (
        BORDER_ID, GROUP_NUM, GROUP_ORDER, GROUP_TAB,
        USER_ID, NICKNAME, TITLE, BORDER_TEXT, WRITE_DAY
    ) VALUES (
        #{borderId},
        #{borderId},  -- [중요] 원글은 자기 번호가 곧 그룹 번호
        0,            -- 원글 순서는 0
        0,            -- 원글 들여쓰기는 0
        #{userId}, #{nickname}, #{title}, #{borderText}, NOW()
    )
</insert>
```
...
**Mapper XML**
```xml
<select id="selectBorderList" resultType="egovMap">
    SELECT
        BORDER_ID, TITLE, NICKNAME, WRITE_DAY, SEE_COUNT,
...
**1) 자리 확보 (UPDATE)**
```xml
<update id="updateOtherReply" parameterType="borderVO">
    UPDATE EGOV_BBS
...
**2) 답글 저장 (INSERT)**
```xml
<insert id="insertBorderReply" parameterType="borderVO">
    INSERT INTO EGOV_BBS (
        BORDER_ID, GROUP_NUM, GROUP_ORDER, GROUP_TAB,
        USER_ID, TITLE, ...
    ) VALUES (
        nextval('egov_bbs_border_id_seq'),
...
public void insertReply(BorderVO vo) {
    borderDAO.updateOtherReply(vo); // 자리 비켜!
    borderDAO.insertBorderReply(vo); // 내 자리 찜!
}
```
...
<update id="updateBorder">
    UPDATE EGOV_BBS
    SET TITLE = #{title}, BORDER_TEXT = #{borderText}
